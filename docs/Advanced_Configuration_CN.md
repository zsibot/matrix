...
本文依据当前 UeSim 源码中的 `UMujocoConfigLibrary`、`FMujocoRobotConfig` 和随项目提供的 JSON 模板校正。普通用户优先通过启动器修改配置；手动编辑适合自动化、批量修改和故障排查。
...
Content/model/config/sensors/*.json
~~~

打包后的常见路径为：

~~~text
Windows/UeSim/Content/model/config/config.json
Linux/UeSim/Content/model/config/config.json
~~~
...
> `mujoco_model` 必须解析到目标平台上真实存在的 MJCF/XML。源码模板可能包含开发机绝对路径；分发或换机时应由启动器重写，或改为目标机器上的有效路径。

## robot 字段

| 字段 | 类型 | 当前含义 |
|---|---|---|
| `robot_type` | string | 机器人/协议标识，例如 `xgb`、`xgw`、`zgws` |
...
| `hardware_simulation` | boolean | 选择 MuJoCo 共享内存模拟硬件链路；该链路当前仅支持 Linux |
...
~~~text
Windows/UeSim/Saved/Config/Windows/GameUserSettings.ini
Linux/UeSim/Saved/Config/Linux/GameUserSettings.ini
~~~
